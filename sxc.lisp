...
(defun make-keyword (symbol)
  (read-from-string
   (concatenate 'string ":" (symbol-name symbol))))
...
(defun generate-arg-names-types-and-declerations (fname
						  args
						  &key
						  (caller-name "unknown"))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;- sxc.lisp - s-expreesion c to c transpiler
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(def list sxc-read-file ((simple-string filename))
  "read the file at filename and return a list of forms, with each
form containing a a combination of the form a it's line number. ie:
...
(def t output-c ((list form)
                 (simple-string filename)
                 &optional
                 (stream s *standard-input*))
  "Output C Code to filename."
  (case (cadr form)
...
(def t sxc ((pathname filename) (stream stream))
  "compile filename containing sxc code to c to stream"
    (dolist (form (sxc-read-file filename))
      (output-c form  filename)))
